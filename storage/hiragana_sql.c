...
{
    char db_name[] = "hiragana.db";
    char filename[] = "../hiragana.txt";
...
{
    char *sql = "CREATE TABLE IF NOT EXISTS hiragana_mode ("
...
int insert_data(char db_name[], char filename[])
{
    ...
    char *sql = "INSERT INTO hiragana_mode (Hiragana, Romaji) VALUES (?, ?);";
    ...
    while (1)
    {
        if (fgets(hiragana, sizeof(hiragana), in_file) == NULL) break;
        if (fgets(romaji,   sizeof(romaji),   in_file) == NULL) break;

        sqlite3_bind_text(stmt, 1, hiragana, -1, SQLITE_TRANSIENT);
